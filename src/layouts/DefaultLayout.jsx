    ...
    <>
      <div className="">
        <aside
          ...
          className={`fixed top-0 bottom-0 left-0 z-40 flex w-64 flex-col border-r border-r-slate-200 bg-white transition-transform sm:translate-x-0 ${isSideNavOpen ? "translate-x-0" : " -translate-x-full"
            }`}
        >
          <SidebarHeader />
          <nav
          ...
          >
            <div>
              <ul className="flex flex-1 flex-col gap-1 py-3">
                {DataSidebar.map((item, idx) => <SidebarItem key={idx} item={item} setIsSideNavOpen={setIsSideNavOpen} />)}
              </ul>
            </div>
          </nav>
