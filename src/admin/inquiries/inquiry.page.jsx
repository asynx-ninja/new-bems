...
const InquiryPage = () => {
  ...
  const getBadge = (status, num) => {
    switch (status.toLowerCase()) {
      case "for review":
        return (
          <span className="bg-yellow-100 text-yellow-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full border border-yellow-300">
            {num}
          </span>
        );
      case "cancelled":
        return (
          <span className="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full border border-gray-500">
            {num}
          </span>
        );
      case "rejected":
        return (
          <span className="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full border border-red-400">
            {num}
          </span>
        );
      case "approved":
    ...
    <>
      ...
      <section className="max-w-full" aria-multiselectable="false">
        <div className="mb-3 flex justify-between items-end">
          <h2
            className="flex flex-col items-start text-3xl font-medium lg:items-center md:flex-row "
          ...
          <Link
            ...
            style={{
              color: theme !== null ? theme.secondary : "rgb(59 130 246)",
            }}
          >
            Go to Archived Inquiries
          </Link>
        </div>
        <ul
        ...
        >
          {status.map((status, idx) => (
            <li key={idx} className="" role="presentation">
              <button
                className={`-mb-px inline-flex h-12 w-full items-center justify-center whitespace-nowrap rounded-t border-b-2 px-6 text-sm font-medium tracking-wide transition duration-300 hover:bg-emerald-50 hover:stroke-emerald-600 focus:bg-emerald-50 focus-visible:outline-none disabled:cursor-not-allowed ${
                  tabSelected.currentTab === idx + 1
                ...
                aria-selected={`${
                  tabSelected.currentTab === idx + 1 ? "true" : "false"
                }`}
                onClick={() =>
                  setTabSelected({ ...tabSelected, currentTab: idx + 1 })
                }
              >
                {getBadge(status, 2)}
                <span>{status}</span>
        ...
        <div className="">
          {data.map((data, idx) => (
            <div
              key={idx}
              className={`${
                tabSelected.currentTab === idx + 1 ? "" : "hidden"
              }`}
              id="tab-panel-1a"
              aria-hidden={`${
                tabSelected.currentTab === idx + 1 ? "true" : "false"
