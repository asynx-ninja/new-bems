...
const Table = ({ filtered }) => {
  ...
  const handleSort = (attr) => {
    ...
    switch (attr) {
      ...
      case "company":
        !sortBy.company
          ? setData((prev) =>
              prev.sort((a, b) => a.company.localeCompare(b.company))
            )
          : setData((prev) =>
              prev.sort((a, b) => b.company.localeCompare(a.company))
            );
        break;
      case "date":
        !sortBy.date
  ...
  const headers = ["name", "title", "company", "date"];
  return (
    <>
      {/*<!-- Component: Underline Table --> */}
      <div className="w-full overflow-auto h-[36.2em] bg-slate-100">
        <table
